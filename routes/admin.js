...
const buildFilters = (q) => {
  const conds = [], vals = [];
  let idx = 1;
  if (q.agent_id)       { conds.push(`o.agent_id=$${idx++}`);             vals.push(q.agent_id); }
  else if (q.agent_name){ conds.push(`o.agent_name ILIKE $${idx++}`);     vals.push(`%${q.agent_name}%`); }
  if (q.date_from)      { conds.push(`o.order_date>=$${idx++}`);          vals.push(new Date(q.date_from)); }
  if (q.date_to)        { const t=new Date(q.date_to); t.setHours(23,59,59,999); conds.push(`o.order_date<=$${idx++}`); vals.push(t); }
  if (q.lead_source)    {
    const leadAliases = {
    ...
    if (list) { conds.push(`o.lead_source = ANY($${idx++})`); vals.push(list); }
...
router.get('/stats', async (req, res) => {
  try {
    const [tot, breakdown, topAgents, totalCustomers] = await Promise.all([
      ...
      query(`SELECT order_status, COUNT(*) AS count FROM orders GROUP BY order_status ORDER BY count DESC`),
      query(`SELECT agent_name, agent_id, COUNT(*) AS order_count FROM orders GROUP BY agent_name,agent_id ORDER BY order_count DESC LIMIT 5`),
...
router.get('/orders', async (req, res) => {
  try {
    const { page=1, limit=50, sort='order_date', order='DESC' } = req.query;
    ...
    const [dataRes, countRes] = await Promise.all([
      query(`
        ...
        GROUP BY o.id,o.order_id,o.order_date,o.order_status,o.agent_id,o.agent_name,
                 o.lead_source,o.first_name,o.last_name,o.street_address,o.city,o.state,
    ...
    res.json({ orders: dataRes.rows, pagination: { total, page:parseInt(page), limit:parseInt(limit), pages:Math.ceil(total/parseInt(limit)) } });
  } catch(err) {
...
router.get('/orders/export', async (req, res) => {
  try {
    ...
    if (!result.rows.length) return res.status(404).json({ error: 'No orders found.' });

    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(result.rows);
    ws['!cols'] = [
      {wch:16},{wch:18},{wch:12},{wch:20},{wch:12},{wch:22},
      {wch:14},{wch:14},{wch:30},{wch:14},{wch:14},{wch:10},{wch:12},
      {wch:16},{wch:26},{wch:24},{wch:6},{wch:10},{wch:12},{wch:12},{wch:20},{wch:30},
    ];
    ...
    res.setHeader('Content-Disposition', `attachment; filename="PayerPortal_Orders_${new Date().toISOString().split('T')[0]}.xlsx"`);
...
router.put('/orders/:order_id', [
  ...
  body('items').optional().isArray({ min:1 }).withMessage('At least 1 item required'),
], async (req, res) => {
  ...
  const {
    order_status, lead_source, tracking_id, notes,
  ...
  try {
    ...
    const sets = [], vals = [];
    let idx = 1;
    const addSet = (col, val) => { sets.push(`${col}=$${idx++}`); vals.push(val); };
    ...
    if (street_address !== undefined) addSet('street_address', street_address);
    ...
    if (sets.length > 1) {
      ...
      const res2 = await client.query(
        `UPDATE orders SET ${sets.join(',')} WHERE order_id=$${idx} RETURNING *;`, vals
      );
      if (!res2.rows.length) { await client.query('ROLLBACK'); return res.status(404).json({ error:'Order not found.' }); }
    }
    ...
    res.json({ message:'Order updated successfully.', order: updated.rows[0] });
  } catch(err) {
...
router.post('/agents', [
  body('agent_id').trim().notEmpty().withMessage('Agent ID required').isLength({ max:50 }),
  ...
  try {
    const exists = await query('SELECT id FROM agents WHERE agent_id=$1', [agent_id]);
    if (exists.rows.length) return res.status(409).json({ error: `Agent ID "${agent_id}" already exists.` });
...
router.put('/agents/:agent_id', [
  ...
  body('password').optional({ checkFalsy:true }).isLength({ min:6 }).withMessage('Min 6 chars'),
], async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });
  ...
  try {
    const sets = [], vals = [];
    let idx = 1;
    if (agent_name  !== undefined) { sets.push(`agent_name=$${idx++}`);  vals.push(agent_name); }
    ...
    if (password)                  { const h=await bcrypt.hash(password,12); sets.push(`password=$${idx++}`); vals.push(h); }
...
router.get('/customers', async (req, res) => {
  try {
    ...
    const offset = (parseInt(page)-1)*parseInt(limit);
    const searchCond = search ? `WHERE c.phone ILIKE $1 OR c.first_name ILIKE $1 OR c.last_name ILIKE $1 OR c.email ILIKE $1` : '';
